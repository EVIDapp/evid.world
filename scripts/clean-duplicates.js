...
// IDs to remove (duplicates with suffixes)
const idsToRemove = new Set([
  ...
  // Remove _new versions (keep base versions)
...
// Track seen titles and find duplicates by title
...
events.forEach((event, index) => {
  ...
  if (seenTitles.has(normalized)) {
    ...
    // Keep the one with _area suffix or better description
    const currentScore = 
    ...
    const firstScore = 
      (firstEvent.id.includes('_area') ? 100 : 0) +
      (firstEvent.radiusKm ? 50 : 0) +
      (firstEvent.desc_long?.length || 0);
    
    if (currentScore > firstScore) {
      // Remove first, keep current
      idsToRemove.add(firstEvent.id);
      titleDuplicates.push({
        removed: firstEvent.id,
        kept: event.id,
        title: event.title
      });
...
console.log(`\nFound ${titleDuplicates.length} title-based duplicates`);
titleDuplicates.forEach(dup => {
  console.log(`  - Removed: ${dup.removed}, Kept: ${dup.kept} (${dup.title})`);
});
